...
class Hla(HighLevelAnalyzer):
    ...
    def __init__(self):
        # Current Bit Width based on bit rate
        self.bit_width = 0
        self.synced = 0
        # Bit Pump
        self.pump_last_width = 0
        self.pump_bits = []
        # Bit Sync
        self.sync_bits = []
        self.sync_start = 0
    ...
    def break_width(self):
        ...
        # but bit length can vary a bit, so we go a little longer 
        # for a margin of error
        return (self.bit_width * 13)

    # ==========================================================================================
    # DECODE
    # ==========================================================================================
    
    # Decode square waves into bits and bytes
    def decode(self, frame: AnalyzerFrame):
        self.frames = []
        self.this_frame = frame

        bit = frame.data['data'][0]
        width = frame.data['length']
        
        if (self.bit_width == 0):
            # We have not yet SYNC'd
            if (self.pump_last_width == 0):
                # This is the start
                self.state_start = self.this_frame.start_time
                self.bit_start()
            else:
                self.bit_sync(bit, width)
        else:
            # We know the bit width
            if (bit==0) and (bit > self.break_width()):
                self.show_state = self.start_time
                self.bit_break()
            else:
                # Have we changed our rate?
                if (width < (self.bit_width * TOO_NARROW)):
                    # This should be a new SYNC to accomidate a new bit rate
                    self.bit_width = 0
                    self.state_start = self.this_frame.start_time
                    self.sync_bits = []
                    self.bit_sync(bit, width)
                else:
                    self.bit_pump(bit, width)

        self.pump_last_width = width
    ...
    def bit_pump(self, bit, width):
        while (width > self.bit_width * TOO_NARROW):
            self.pump_bits.append(bit)
            width -= self.bit_width
            if (len(self.pump_bits) == 1):
                # First bit, start the new state pointer
                self.state_start = self.this_frame.start_time
            elif (len(self.pump_bits) == 12):
                # Finished a byte, process it and keep moving
                self.byte_pump(self.pump_bits, width)
                self.pump_bits = []
                if (bit == 1):
                    # What's left of this bit is now an IDLE
                    break

    def bit_sync(self, bit, width):
        if len(self.sync_bits) == 0:
            if (bit == 0):
                # Bit 0 LOW starts sync 
                self.sync_width = width
                self.sync_bits = []
                self.sync_bits.append(bit)
                self.state_start = self.this_frame.start_time
            else:
                # Bit 0 HIGH is an idle
                self.state_start = self.this_frame.start_time
                self.bit_idle()
        elif len(self.sync_bits) < 8:
            # Bits 1,2,3,4,5,6,7 should all alternate but be same length
            if (bit != (len(self.sync_bits) & 0x01)):
                # Wrong bit value
                self.sync_bits = []
                self.sync_error(BitErrors.NOTSYNC)
            elif width > self.pump_last_width * TOO_WIDE:
                # Too wide
                self.sync_bits = []
                self.sync_error(BitErrors.WIDE)
            elif width < self.pump_last_width * TOO_NARROW:
                # Too Narrow
                self.sync_bits = []
                self.sync_error(BitErrors.NARROW)
            else:
                self.sync_bits.append(bit)
                self.sync_width += width
        elif len(self.sync_bits) <10:
            # Bits 8 and 9 (Parity) should be LOW
            if (bit != 0):
                # Wrong value
                self.sync_bits = []
                self.sync_error(BitErrors.NOTSYNC)
            elif width > (self.pump_last_width * TOO_WIDE * 2):
                # Too Wide for a double width 0
                self.sync_bits = []
                self.sync_error(BitErrors.WIDE)
            elif width < (self.pump_last_width * TOO_NARROW * 2):
                # Too Narrow for a double width 0
                self.sync_bits = []
                self.sync_error(BitErrors.NOTSYNC)
            else:
                self.sync_bits.append(bit)
                self.sync_bits.append(bit)
                self.sync_width += width
        elif len(self.sync_bits) < 11:
            # Bits 9 and 10 (stop bits) should be HIGH
            if (bit != 1):
                # Wrong value
                self.sync_bits = []
                self.sync_error(BitErrors.NOTSYNC)
