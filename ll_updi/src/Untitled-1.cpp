...
FrameFlags ll_analyzer::Valid( BitState bit, U64 Width = 0 )
{
...
void ll_analyzer::Identify( U64 start, U64 end, const char* note, FrameFlags flag, int value, const char* comment )
{
    ...
    frameV2.AddString( "comments", comment );
    mResults->AddFrameV2( frameV2, note, start, end );
}


void ll_analyzer::Identify( const char* note, FrameFlags flag, int value, const char* comment )
{
    Identify( channel->GetSampleNumber(), channel->GetSampleOfNextEdge(), note, flag, value );
}

void ll_analyzer::Identify( U64 start_time, const char* note, FrameFlags flag, int value, const char* comment )
{
...
bool ll_analyzer::unsynced()
{
    ...
    // Default is that we are still unsynced
    mResults->AddMarker( channel->GetSampleNumber() + ( bit_rate / 2 ), AnalyzerResults::X, mSettings->mInputChannel );
    sync_bit_count = 0;
    sync_start = 0;
    return true;
}
        ...
        while( true )
        {
            ...
            case 12:
                ...
                if( data_value == 0x55 )
                {
                    ...
                    Identify( data_start_bit, "SYNC", FLAG_SYNC, data_value );
                }
                else
                {
